...
class Bootstrap_Image_Helper {
	...
	public static function renderImage(
		int|object|array $attachment,
		string $class = '',
		array $viewport_sizes = array(),
		array $video_options = array(),
	): string {
	 ...
	 */
	private static function buildFormatSrcsets( int $attachment_id, array $intermediate_sizes ): array {
		$srcsets = array(
			'avif'     => '',
			'webp'     => '',
			'original' => '',
	 ...
	 */
	private static function getModernFormatUrl( string $original_url, string $format, string $base_dir, string $base_name, string $size_name, array $metadata ): string|false {
	 ...
	 */
	public static function getAttachmentIdFromUrl( string $url ): int|false {
		global $wpdb;
